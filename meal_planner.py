...
def solve_milp(
    ...
    def build_constraints(
        ...
        # FIX 5: Component exclusion — also covers a_la_carte items' own components
        bigM = M
        ...
        alac_name_map = {df.loc[f_idx, "menu_item"]: i for i, f_idx in enumerate(F)}  # same as food_name_map

        # Gather all items (combos AND a_la_carte) that have sub-components
        # Combos: exclusion via z binary linking variable
        ...
        # A_la_carte items: if an a_la_carte item's sub-components appear in F,
        # prevent selecting both the item and its components simultaneously.
        for k, f_idx in enumerate(F):
            components = []
            for cc in ["carbs", "item_1", "item_2", "item_3", "drink"]:
                val = df.loc[f_idx, cc]
                if val and val != "none":
                    components.append(val)
            for comp in components:
                if comp in alac_name_map and alac_name_map[comp] != k:
                    r = np.zeros(nVars)
                    r[ix[k]] = 1
                    r[ix[alac_name_map[comp]]] = 1
                    add_ineq(r, max_duplicates)
    ...
    if sol is None:
        # Try iterative relaxations: 10%, 20%, 30%
        for pct in [0.10, 0.20, 0.30]:
            for step_name, relaxer in [
                ("Min Calories ↑",   lambda p: (cur_min_cals * (1 - p), cur_min_protein, cur_max_sodium, cur_weekly_budget)),
                ("Min Protein ↑",    lambda p: (cur_min_cals, cur_min_protein * (1 - p), cur_max_sodium, cur_weekly_budget)),
                ("Max Sodium ↓",     lambda p: (cur_min_cals, cur_min_protein, cur_max_sodium * (1 + p), cur_weekly_budget)),
                ("Budget ↑",  lambda p: (cur_min_cals, cur_min_protein, cur_max_sodium, cur_weekly_budget * (1 + p))),
            ]:
                trial_cals, trial_prot, trial_sod, trial_budget = relaxer(pct)
                ...
                if trial_sol is not None:
                    ...
                    relaxations_applied.append(f"{step_name} by {int(pct*100)}%")
                    break
            if sol is not None:
                break
...
<hr style="border:none;border-top:2px solid #7b1113;margin:10px 0 20px 0">""", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        with settings_tab:
            ...
            selected_days_m = st.multiselect(
            ...
            max_sugar_m   = st.number_input(rdv_label("max_sugar"),   0, 5000,  700,   step=50,  key="sugar_m",   help=rdv_help("max_sugar"))
            max_fat_m     = st.number_input(rdv_label("max_fat"),     0, 2000,  400,   step=50,  key="fat_m",     help=rdv_help("max_fat"))
            max_sodium_m  = st.number_input(rdv_label("max_sodium"),  0, 50000, 12000, step=500, key="sodium_m",  help=rdv_help("max_sodium"))
            min_protein_m = st.number_input(rdv_label("min_protein"), 0, 500,   20,    step=5,   key="protein_m", help=rdv_help("min_protein"))
            min_cals_m    = st.number_input(rdv_label("min_cals"),    0, 30000, 7000,  step=500, key="cals_m",    help=rdv_help("min_cals"))

            st.markdown("### 🔁 Duplicate Limit")
            max_items_m = max(1, n_days_m * meals_per_day_m)
            max_dup_m = st.slider(
                "Max times same item can appear",
                1, max_items_m, min(2, max_items_m),
                help="1 = unique items only; higher = allow repeats",
                key="dup_m",
            )

            st.markdown("### 📍 Allowed Kiosks")
            selected_locs_m = st.multiselect("Select kiosks", options=loc_options, default=loc_options, key="locs_m")
        ...
        with smart_tab:
            st.markdown("### 📅 Schedule")
            selected_days_s = st.multiselect(
                "Select days on campus",
                ALL_DAYS,
                default=["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"],
                key="days_s",
            ...
            max_dup_s = st.slider(
                "Max times same item can appear",
                1, max_items_s, min(2, max_items_s),
                help="1 = unique items only; higher = allow repeats",
                key="dup_s",
            )

            st.markdown("### 📍 Allowed Kiosks")
            selected_locs_s = st.multiselect("Select kiosks", options=loc_options, default=loc_options, key="locs_s")

            st.markdown("---")
            
            # ── PROFILE & CALCULATOR SECTION ──────────────────────────────────
            st.markdown("### 🧬 Preset Profiles")
            st.caption("Choose a preset or use the personalised calculator below.")

            selected_profile = st.radio(
                "Quick profile",
                options=["None"] + list(PROFILES.keys()),
                format_func=lambda x: f"{PROFILES[x]['emoji']} {x}" if x != "None" else "✏️ None (use calculator below)",
                key="profile_sel",
            )
            ...
            # FIX 1: Compute nutrient defaults from profile OR calculator
            # Both will reactively update because they're computed inline
            
            if selected_profile != "None":
                # Preset profile selected
                p = PROFILES[selected_profile]
                min_cals_s    = p["min_cals"]
                min_protein_s = p["min_protein"]
                max_sodium_s  = p["max_sodium"]
                max_sugar_s   = p["max_sugar"]
                max_fat_s     = p["max_fat"]
                weekly_budget_s = p["weekly_budget"]
                def_cal_targets = p["cal_targets"]
                
                st.info(p["desc"])
                st.markdown("---")
                st.markdown("### 📊 Auto-Calculated Targets")
                st.caption("🔄 Using preset profile. These values update automatically.")
                ...
                st.markdown("---")
                st.markdown("### 📊 Auto-Calculated Targets")
                st.caption("🔄 Using personalised calculator. These values update automatically when you change your details.")
  ...
  <p style="margin:0;font-size:0.9rem;color:#c62828 !important;"><strong>Auto-relaxation was attempted (10%, 20%, 30%) but could not find a feasible solution.</strong></p>
